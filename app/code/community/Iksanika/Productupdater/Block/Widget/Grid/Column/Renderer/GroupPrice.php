...
//        $this->f->$method($args[0]);
    ...
    public function render(Varien_Object $row) 
    {
        ...
        $html .= '
                <tbody id="group_price_'.$_htmlId.'_container"></tbody>

                <tfoot>
                    <tr>
                        <td style="display:none"></td>
                        <td colspan="4" class="a-right"><button id="id_add_group_price" title="'.Mage::helper('catalog')->__('Add Group Price').'" type="button" class="scalable add" onclick="return groupPriceControl'.$_htmlId.'.addItem()" style=""><span>'.Mage::helper('catalog')->__('Add Group Price').'</span></button></td>
                    </tr>
                </tfoot>
            </table>
        ';
...
var groupPriceControl'.$_htmlId.' = {
    ...
    addItem : function () {
        ...
        Element.insert($(\'group_price_'.$_htmlId.'_container\'), {
            bottom : this.template.evaluate(data)
        });
        $j(\'#product'.$_htmlId.' #group_price_row_\'+data.index+\'_cust_group\').val(data.group);
        $j(\'#product'.$_htmlId.' #group_price_row_\'+data.index+\'_website\').val(data.website_id);
        ...
        '.(($_readonly) ? '
        $(\'group_price_'.$_htmlId.'_container\').select(\'input\', \'select\').each(this.disableElement);
        $(\'group_price_'.$_htmlId.'_container\').up(\'table\').select(\'button\').each(this.disableElement);
        ':' 
        $(\'group_price_'.$_htmlId.'_container\').select(\'input\', \'select\').each(function(el){ Event.observe(el, \'change\', el.setHasChanges.bind(el)); });
        '). '
    },
...
//        foreach($groupPrices as $groupPrice)
        ...
        {
            $html .= '
                    groupPriceControl'.$_htmlId.'.addItem(\''.$_item['website_id'].'\', \''.$_item['cust_group'].'\', \''.sprintf('%.2f', $_item['price']).'\', '.(int)!empty($_item['readonly']).');';
        }
        if ($_readonly)
        {
            $html .= '$(\'group_price_'.$_htmlId.'_container\').up(\'table\').select(\'button\')
