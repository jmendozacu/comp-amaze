...
/*  cookieDays: 365,
  ...
  init: function(tableId, saveRepositionUrl) {
      
      this.tableId = tableId;
      this.saveRepositionUrl = saveRepositionUrl;
    // Don't initialize twice
//console.debug('test #1 ' + arguments.callee.done);
...
/*******/
        ...
        $$('.headings th').each(function(th, i) {
            if (i == targetCol)
            {
                th.addClassName('dragtable_marker_position');
            } else
            {
                th.removeClassName('dragtable_marker_position');
            }
        });
...
*/
          console.debug('rememberDrag: ' + dragObj.table.id + ' ' + dragObj.startCol + ' ' + targetCol);
        dragtable.rememberDrag(dragObj.table.id, dragObj.startCol, targetCol);
//      if (dragObj.table.id && dragObj.table.className.search(/\bforget-ordering\b/) == -1) 
        {
            console.debug('rememberDrag');
            dragtable.rememberDrag(dragObj.table.id, dragObj.startCol, targetCol);
        }
...
//            console.debug('1: #' + this.tableGridId + ' .headings th');
//            console.debug('2: #' + this.tableId + ' .headings th');
...
//                    postData += '&fields[]=' + el.getAttribute('colname');
            ...
            // save columns order
            new Ajax.Request(this.saveRepositionUrl, {
                method: 'post',
                postBody : postData,
